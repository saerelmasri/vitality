...
const addProfilePhoto = async (req, res) => {
  ...
  const insertImage = process.env.BASE_URL + `public/coaches/${req.file.filename}`;
  ...
  try {
    ...
    const query = `
      INSERT INTO coach_photo (coach_id, photo_url)
      VALUES (?, ?)
...
const getLastPhotoUrl = async (req, res) => {
  ...
  try {
    const query = 'SELECT photo_url FROM coach_photo WHERE coach_id = ? ORDER BY id DESC LIMIT 1';
